...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	// TODO: IMPLEMENT ME
	sim.snapshotDone.Store(snapshotId, make(chan string, len(sim.servers)))
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	// TODO: IMPLEMENT ME
	sd,ok := sim.snapshotDone.Load(snapshotId)
	if !ok {
		log.Fatal("cant find snapshot associated with id ",snapshotId)
	}
	sdc := sd.(chan string)
	sdc <- serverId
...
func (sim *Simulator) CollectSnapshot(snapshotId int) *SnapshotState {
	mergedSnapshot := SnapshotState{snapshotId,make(map[string]int),make([]*SnapshotMessage, 0)}
	sd,_ := sim.snapshotDone.Load(snapshotId)
	sdc := sd.(chan string)
	...
	for _, server := range sim.servers {
		snapshotState,_ := server.completedSnapshots.Load(snapshotId)
		snapshot := snapshotState.(*SnapshotState)
		mergedSnapshot.tokens[server.Id] = snapshot.tokens[server.Id]
		//loop over messages
		for _, msg := range snapshot.messages{
			mergedSnapshot.messages = append(mergedSnapshot.messages, msg)
		}
	}
	sim.snapshotDone.Store(snapshotId, sdc)
